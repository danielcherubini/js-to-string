...
test("Empty Function", t => {
    ...
    const expected = `{data: function() { return {}; }}`;
    t.is(result, expected);
});
...
test("Merged Function", t => {
    ...
    const expected = `function() { return {"msg":"Hello world!","messageOuter":"Say Foo","foo":true}; }`;
    t.is(result, expected);
});

test("Not Function", t => {
    const result = stringit(notEmpty);
    const expected = `{data: function() { return {"msg":"Hello world!","messageOuter":"Say Foo"}; }}`;
    t.is(result, expected);
});
